...
{
    using BreLaneIndex = GryboHighwayOrderingHelpers.BreLaneIndex;
    ...
    public sealed class FiveLaneKeysPlayer : TrackPlayer<FiveLaneKeysEngine, GuitarNote>
    {
        ...
        private Dictionary<FiveLaneKeysAction, BreLaneIndex> _actionToBreLaneIndex = new();

        // When a BRE lane element needs to know how bright it should be, it'll use this table to get the right BRE lane index
        private Dictionary<int, BreLaneIndex> _highwayOrderingIndexToBreLaneIndex;

        // Record of the most recent time that each BRE lane has been lit up by any of the actions that map to it
        private Dictionary<BreLaneIndex, double> _breLaneIndexToMostRecentTime = new();
        ...
        protected override void OnCodaStart(CodaSection coda)
        {
            ...
            CurrentCoda.SetLaneIndexes(new()
            {
                ...
                {(int)FiveLaneKeysAction.OpenNote, (int)BreLaneIndex.Green },
                {(int)FiveLaneKeysAction.GreenKey, (int)BreLaneIndex.Green },
                {(int)FiveLaneKeysAction.RedKey, (int)BreLaneIndex.Green },
                {(int)FiveLaneKeysAction.YellowKey, (int)BreLaneIndex.Green },
                {(int)FiveLaneKeysAction.BlueKey, (int)BreLaneIndex.Green },
                {(int)FiveLaneKeysAction.OrangeKey, (int)BreLaneIndex.Green },
            });
        ...
        {
            UsingOpenLane = GryboHighwayOrderingHelpers.ShouldUseOpenLane(Player.Profile.OpenLaneDisplayType, NoteTrack.Notes);

            if (UsingOpenLane)
            {
                LaneCount = 6;
                _highwayOrdering = GryboHighwayOrderingHelpers.OPEN_LANE_HIGHWAY_ORDERING;
                _actionToBreLaneIndex = new()
                {
                    { FiveLaneKeysAction.OpenNote, BreLaneIndex.Open },
                    { FiveLaneKeysAction.GreenKey, BreLaneIndex.Green },
                    { FiveLaneKeysAction.RedKey, BreLaneIndex.Red },
                    { FiveLaneKeysAction.YellowKey, BreLaneIndex.Yellow },
                    { FiveLaneKeysAction.BlueKey, BreLaneIndex.Blue },
                    { FiveLaneKeysAction.OrangeKey, BreLaneIndex.Orange },
                };

                _highwayOrderingIndexToBreLaneIndex = new()
                {
                    { _highwayOrdering[(int)FiveFretGuitarFret.Open], BreLaneIndex.Open },
                    { _highwayOrdering[(int)FiveFretGuitarFret.Green], BreLaneIndex.Green },
                    { _highwayOrdering[(int)FiveFretGuitarFret.Red], BreLaneIndex.Red },
                    { _highwayOrdering[(int)FiveFretGuitarFret.Yellow], BreLaneIndex.Yellow },
                    { _highwayOrdering[(int)FiveFretGuitarFret.Blue], BreLaneIndex.Blue },
                    { _highwayOrdering[(int)FiveFretGuitarFret.Orange], BreLaneIndex.Orange },
                };
            }
            else
            {
                LaneCount = 5;
                _highwayOrdering = GryboHighwayOrderingHelpers.DEFAULT_HIGHWAY_ORDERING;
                _actionToBreLaneIndex = new()
                {
                    { FiveLaneKeysAction.OpenNote, BreLaneIndex.Green },
                    { FiveLaneKeysAction.GreenKey, BreLaneIndex.Green },
                    { FiveLaneKeysAction.RedKey, BreLaneIndex.Red },
                    { FiveLaneKeysAction.YellowKey, BreLaneIndex.Yellow },
                    { FiveLaneKeysAction.BlueKey, BreLaneIndex.Blue },
